...
export function Login(){
    ...
    async function handleLogin(){
        if(email!=='' && password!==''){
            let salt;
            let responseDataSalt;
            if(!EmailValidator.validate(email)){
                alert("Invalid Email");
                return;
            }
            try{
                // Send the POST request using fetch to get salt
                const response = await fetch(server+"api/users/salt", {
                    method: "POST",
                    headers: {
                ...
                responseDataSalt = await response.json();
                if (!response.ok) {
                    throw new Error("Error: " + response.status);
                }
                else{
                    console.log(responseDataSalt.salt);
                    salt = responseDataSalt.salt;
                    setCookie('salt',salt,15);
                }
            }
            catch(error){
                if(responseDataSalt==undefined){
                    alert(error.message+" Failed to reach server");
                }
                else{
                    alert(responseDataSalt.message);
                }
